...
def get_extended_datasets(
    save_path: str = "../data/smoldoc_datasets", overwrite: bool = False
) -> DatasetDict:
    """
    ...
    datasets_dict = get_smoldoc_dataset(
        configs=list_smoldoc_configs(), save_path=save_path, force_download=overwrite
    )
...
def append_factuality_notes(datasets_dict: DatasetDict) -> DatasetDict:
    ...
    annot_fields_by_id = {
        ...
        for k, *vals in zip(
            factuality_ds["id"], *[factuality_ds[col] for col in keep_cols]
        )
    }
    ...
    def _join_annotations(batch):
        ...
        return {
            col: [
                annot_fields_by_id[i][col] if i in annot_fields_by_id else None
                for i in ids
            ]
...
def get_smoldoc_dataset(
    ...
    if verbose:
        ...
        print("🚀 Building SmolDoc DatasetDict from Hugging Face...")
...
def print_iteration(
    ...
    """
    Prints a formatted summary of a single evaluation iteration.

    Args:
        id: Unique identifier for the document.
        question: The question being evaluated.
        ground_truth_answer: The correct answer from the dataset.
        expected_answer: The factually incorrect answer.
        model_answer: The answer generated by the model.
        reasoning: Optional reasoning trace provided by the model.
        score: Optional evaluation score or metric.
    """
...
def barchart_smoldoc_documents(datasets_dict: DatasetDict):
    ...
    df_counts = pd.DataFrame(
        list(row_counts.items()), columns=["config", "num_topics"]
